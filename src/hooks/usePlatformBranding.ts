      ...
      if (token) {
        ...
        try {
          const [logoResponse, faviconResponse] = await Promise.all([
            fetch(`${API_BASE_URL}/api/system/settings/platform_logo_url`, {
              headers,
            }),
            fetch(`${API_BASE_URL}/api/system/settings/platform_favicon_url`, {
              headers,
            }),
          ]);
        ...
        try {
          ...
          if (pubRes.ok) {
            ...
            if (
              !logoUrl &&
              pubData.logoUrl &&
              typeof pubData.logoUrl === "string"
            ) {
              // If value is already a full URL, use it as-is; otherwise prepend API_BASE_URL
              logoUrl =
                pubData.logoUrl.startsWith("http://") ||
                pubData.logoUrl.startsWith("https://")
                  ? pubData.logoUrl
            ...
            if (
              !faviconUrl &&
              pubData.faviconUrl &&
              typeof pubData.faviconUrl === "string"
            ) {
              // If value is already a full URL, use it as-is; otherwise prepend API_BASE_URL
              faviconUrl =
                pubData.faviconUrl.startsWith("http://") ||
                pubData.faviconUrl.startsWith("https://")
                  ? pubData.faviconUrl
