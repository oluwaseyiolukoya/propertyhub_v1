...
async function request<T>(
  ...
  try {
    console.log(
      `[request] Making ${
        options.method || "GET"
      } request to: ${API_BASE_URL}${endpoint}`
    );
    ...
    console.log(
      `[request] Response status: ${response.status} ${response.statusText}`
    );
    ...
    if (response.status === 401 && !extra?.suppressAuthRedirect) {
      if ((data as any)?.code === "PERMISSIONS_UPDATED") {
        ...
        (data as any)?.code === "SESSION_REVOKED" ||
        (data as any)?.code === "SESSION_EXPIRED"
      ) {
    ...
    if (!response.ok) {
      ...
      if (
        ...
        ((data as any)?.code === "ACCOUNT_BLOCKED" ||
          /deactivated|blocked/i.test(((data as any)?.error || "") as string))
      ) {
        const event = new CustomEvent("accountBlocked", {
          detail: {
            message:
              (data as any)?.error || "Your account has been deactivated",
          },
        });
        window.dispatchEvent(event);
      }
      return {
        error: {
          ...
          ...(typeof (data as any)?.kycRequired !== "undefined"
            ? { kycRequired: (data as any).kycRequired }
            : {}),
          ...(typeof (data as any)?.kycStatus !== "undefined"
            ? { kycStatus: (data as any).kycStatus }
            : {}),
          ...(typeof (data as any)?.kycFailureReason !== "undefined"
            ? { kycFailureReason: (data as any).kycFailureReason }
            : {}),
        },
      };
    }
