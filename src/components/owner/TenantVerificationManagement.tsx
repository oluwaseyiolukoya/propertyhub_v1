...
export const TenantVerificationManagement: React.FC = () => {
  ...
  const [analytics, setAnalytics] =
    useState<TenantVerificationAnalytics | null>(null);
  ...
  const [tenantToDelete, setTenantToDelete] =
    useState<TenantVerification | null>(null);
  ...
  const loadTenants = async () => {
    try {
      setLoading(true);
      const response = await getTenantVerifications(
        filter,
        page,
        20,
        searchTerm
  ...
  const handleReject = async () => {
    ...
    try {
      setActionLoading(true);
      const response = await rejectTenantKyc(
        selectedTenant.id,
        rejectionReason
      );
      if (response.data?.success) {
  ...
  const handleRequestResubmit = async () => {
    ...
    try {
      setActionLoading(true);
      const response = await requestKycResubmit(
        selectedTenant.id,
        resubmitReason
      );
      if (response.data?.success) {
        ...
        toast.error(
          response.error?.message || "Failed to request resubmission"
        );
      }
    } catch (error: any) {
  ...
  const handleDeleteVerification = async () => {
    ...
    try {
      setActionLoading(true);
      const response = await deleteTenantVerification(
        tenantToDelete.id,
        deleteReason
      );
      if (response.data?.success) {
  ...
  const handleViewDocument = async (document: VerificationDocument) => {
    ...
    try {
      ...
      const response = await getTenantDocumentUrl(
        selectedTenant.id,
        document.id
      );
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-purple-100 mt-1">
                Review and approve tenant KYC submissions
              </p>
            </div>
          </div>
          <Button
          ...
          <Card className="border-0 shadow-md hover:shadow-lg transition-shadow">
            <CardContent className="pt-6">
              <div className="flex flex-col items-center text-center gap-2">
                ...
                <div>
                  <p className="text-3xl font-bold text-gray-900">
                    {analytics.summary.pendingReview}
                  </p>
                  <p className="text-sm text-gray-600 font-medium mt-1">
                    Pending Review
                  </p>
          ...
          <Card className="border-0 shadow-md hover:shadow-lg transition-shadow">
            <CardContent className="pt-6">
              <div className="flex flex-col items-center text-center gap-2">
                ...
                <div>
                  <p className="text-3xl font-bold text-gray-900">
                    {analytics.summary.ownerApproved}
                  </p>
                  <p className="text-sm text-gray-600 font-medium mt-1">
                    Approved
                  </p>
          ...
          <Card className="border-0 shadow-md hover:shadow-lg transition-shadow">
            <CardContent className="pt-6">
              <div className="flex flex-col items-center text-center gap-2">
                ...
                <div>
                  <p className="text-3xl font-bold text-gray-900">
                    {analytics.summary.ownerRejected}
                  </p>
                  <p className="text-sm text-gray-600 font-medium mt-1">
                    Rejected
                  </p>
            ...
            <div className="text-center p-12">
              ...
              <p className="text-gray-500 text-sm mt-2">
                {filter !== "all"
                  ...
                  : "Add tenants to see their verification status"}
              </p>
            </div>
          ) : (
                          ...
                          >
                            ...
                            {tenant.kycVerificationId &&
                              (tenant.kycStatus === "in_progress" ||
                                ...
                                tenant.kycStatus === "pending_documents") && (
                                <DropdownMenuItem
                            ...
                            {tenant.ownerApprovalStatus === "pending" &&
                              tenant.kycStatus === "in_progress" && (
                                <>
                                  <DropdownMenuItem
                                    onClick={() => {
                            ...
                            {(tenant.ownerApprovalStatus === "rejected" ||
                              tenant.kycStatus === "rejected") && (
                              <DropdownMenuItem
                                onClick={() => {
                                  handleViewDetails(tenant.id);
      ...
      <Dialog open={showDetailModal} onOpenChange={setShowDetailModal}>
        <DialogContent className="max-w-2xl max-h-[85vh] border-0 shadow-2xl overflow-hidden flex flex-col">
          <DialogHeader className="bg-gradient-to-r from-[#7C3AED] to-[#5B21B6] -mx-6 -mt-6 px-6 py-4 rounded-t-xl flex-shrink-0">
            ...
            <div className="space-y-6 mt-4 overflow-y-auto flex-1 min-h-0 pr-1">
              ...
              <div className="flex items-center gap-4 bg-gradient-to-br from-purple-50 to-purple-100/50 p-4 rounded-xl border-2 border-purple-200">
                ...
                <div className="ml-auto">
                  {getStatusBadge(
                    selectedTenant.kycStatus,
                    selectedTenant.ownerApprovalStatus
                  )}
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="bg-white p-3 rounded-lg border border-purple-200">
                    ...
                    <p className="font-bold text-gray-900 capitalize">
                      {selectedTenant.kycStatus?.replace("_", " ") || "Pending"}
                    </p>
                  </div>
                  <div className="bg-white p-3 rounded-lg border border-purple-200">
                    <p className="text-gray-500 text-xs font-semibold mb-1">
                ...
                {selectedTenant.ownerNotes && (
                  <div className="mt-4 p-3 bg-gradient-to-br from-blue-50 to-blue-100/50 border-2 border-blue-200 rounded-xl">
                    <p className="text-sm text-blue-800 font-medium">
                      <strong className="font-bold">Owner Notes:</strong>{" "}
                      {selectedTenant.ownerNotes}
                    </p>
                  </div>
                )}
              ...
              {(!selectedTenant.documents ||
                selectedTenant.documents.length === 0) &&
                selectedTenant.kycStatus === "in_progress" && (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-amber-700 mt-1">
                          The tenant is in the process of uploading their
              ...
              {/* Actions */}
              {selectedTenant.ownerApprovalStatus === "pending" &&
                selectedTenant.kycStatus === "in_progress" && (
              ...
              {(selectedTenant.ownerApprovalStatus === "rejected" ||
                selectedTenant.kycStatus === "rejected") && (
                <div className="flex gap-3 justify-end pt-4 border-t">
                  <Button
                    variant="outline"
        ...
        <DialogContent className="border-0 shadow-2xl">
          <DialogHeader className="bg-gradient-to-r from-red-600 to-red-700 -mx-6 -mt-6 px-6 py-4 rounded-t-xl">
            ...
            <DialogDescription className="text-red-100">
              Please provide a reason for rejecting {selectedTenant?.name}'s
              verification
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 mt-4">
      ...
      <Dialog open={showDeleteModal} onOpenChange={setShowDeleteModal}>
        <DialogContent className="border-0 shadow-2xl">
          <DialogHeader className="bg-gradient-to-r from-red-600 to-red-700 -mx-6 -mt-6 px-6 py-4 rounded-t-xl">
            ...
            <DialogDescription className="text-red-100">
              This will delete {tenantToDelete?.name}'s verification request and
              reset their KYC status. The tenant will need to submit a new
            ...
            <div className="p-4 bg-gradient-to-br from-amber-50 to-amber-100/50 border-2 border-amber-200 rounded-xl">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-amber-700 mt-1">
                    This action will permanently delete all uploaded documents
                    and verification history for this tenant.
                  </p>
                </div>
              </div>
            </div>
      ...
      <KYCVerificationDialog
        ...
        onOpenChange={setShowVerifyKYCModal}
