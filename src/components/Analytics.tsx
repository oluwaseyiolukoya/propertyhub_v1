...
export function Analytics() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      if (overviewRes.error)
        toast.error(
          overviewRes.error.error || "Failed to load analytics overview"
        );
      if (dashboardRes.error)
        toast.error(
          dashboardRes.error.error || "Failed to load analytics dashboard"
        );
      if (healthRes.error)
        toast.error(healthRes.error.error || "Failed to load system health");
      if (logsRes.error)
        toast.error(logsRes.error.error || "Failed to load activity logs");
      if (customersRes.error)
        toast.error(customersRes.error.error || "Failed to load customers");
      if (customerAnalyticsRes.error)
        toast.error(
          customerAnalyticsRes.error.error ||
      ...
      if (customerAnalyticsRes.data) {
        console.log(
          "✅ Setting customer analytics data:",
  ...
  let apiTotalCustomers = toNumber(
    overview?.totalCustomers ?? dashboardData?.overview?.totalCustomers
  );
  let apiActiveCustomers = toNumber(
    overview?.activeCustomers ?? dashboardData?.overview?.activeCustomers
  );
  let apiTotalProperties = toNumber(
    overview?.totalProperties ?? dashboardData?.overview?.totalProperties
  );
  let apiTotalUsers = toNumber(
    overview?.totalUsers ?? dashboardData?.overview?.totalUsers
  );
  ...
  const customersCountFallback = Array.isArray(customers)
    ? customers.length
    : 0;
  const activeCustomersFallback = Array.isArray(customers)
    ? customers.filter((c: any) =>
        ["active", "trial"].includes((c.status || "").toLowerCase())
      ).length
    : 0;
  const totalPropertiesFallback = Array.isArray(customers)
    ? customers.reduce(
        (sum: number, c: any) =>
          sum + toNumber(c.propertiesCount ?? c._count?.properties),
        0
      )
    : 0;
  ...
  const mrrFallback = Array.isArray(customers)
    ? customers.reduce(
        (sum: number, c: any) => sum + toNumber(c.mrr ?? c.plan?.monthlyPrice),
        0
      )
    : 0;
  ...
  const acquisitionCount = dailyStats.reduce(
    (s, d: any) => s + (Number(d.customers) || 0),
    0
  );
  const arpu =
    activeCustomers > 0
      ? Math.round((mrr / activeCustomers) * 100) / 100
      : null;
  ...
  const ltv =
    arpu !== null && customerChurn.rate !== null && customerChurn.rate > 0
  ...
  const countryAgg = customers.reduce(
    (acc: Record<string, { customers: number; revenue: number }>, c: any) => {
  ...
  const revenueByPlanName = customers.reduce(
  ...
  const planPerf = (dashboardData?.planDistribution || []).map((pd: any) => ({
    ...
    percentage: totalCustomers
      ? Math.round(((pd.count || 0) / totalCustomers) * 100)
      : 0,
  }));
  ...
  const handleGenerateCustomReport = async () => {
    try {
      ...
      for (const inv of invoices) {
        try {
          ...
          if (
            ...
            revenueByDate[key].currency =
              inv.currency || revenueByDate[key].currency;
          }
        } catch {}
      }
      ...
      const safeName = (
        reportName || `custom-report-${reportMetric}-${reportRange}`
      )
        .replace(/\s+/g, "-")
        .toLowerCase();
      ...
      if (reportMetric === "revenue") {
        ...
        rows.push({
          section: "customers_overview",
          totalCustomers: ov.totalCustomers ?? 0,
          activeCustomers: ov.activeCustomers ?? 0,
          trialCustomers: ov.trialCustomers ?? 0,
        });
        // daily new customers
        const ds = (ovr.dailyStats || []) as Array<any>;
        rows = rows.concat(
          ds.map((d) => ({
            section: "daily_new_customers",
            date: d.date,
            newCustomers: d.customers,
          }))
        );
        ...
        const customerRows = custList.map((c: any) => ({
          ...
          usersCount:
            c._count?.users ?? (Array.isArray(c.users) ? c.users.length : 0),
          country: c.country,
          createdAt: c.createdAt,
          updatedAt: c.updatedAt,
        }));
        ...
        rows.push({
          ...
          latencyMs: sys.database?.latency ?? null,
          connections: sys.database?.connections ?? null,
          dbSize: sys.database?.size ?? null,
          errorLogs24h: sys.errorLogs ?? 0,
          timestamp: sys.timestamp || null,
        });
      } else if (reportMetric === "plan_distribution") {
        const pd = (dsh.planDistribution || []) as Array<any>;
        rows = pd.map((p) => ({
          section: "plan_distribution",
          planId: p.planId,
          planName: p.planName,
          customers: p.count,
        }));
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-center py-16">
          <div className="text-center max-w-md">
            ...
            <p className="mt-2 text-gray-600">
              Once customers and invoices are created, your analytics will
              appear here. You can also try changing the date range.
            </p>
            <div className="mt-4">
      ...
      <div className="bg-gradient-to-r from-purple-50 via-violet-50 to-purple-50 border-b border-purple-200/50 rounded-t-xl">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <Card className="border-0 shadow-xl overflow-hidden">
                    ...
                    {systemHealth && (
                      <div className="mt-3 flex items-center gap-2">
                        <span className="text-xs text-gray-600">
                          {systemHealth.environment} • mode: {systemHealth.mode}
                          {systemHealth.cached ? " • cached" : ""}
                        </span>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium">
                      {typeof systemHealth?.database?.latency === "number" &&
                      systemHealth.database.latency !== null
                    ...
                    {geographicData.map((region, index) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          ...
                          <div
                            className={`flex items-center space-x-1 text-xs ${getGrowthColor(
                              region.growth
                            )}`}
                          >
                ...
                <CardHeader className="p-0">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <CardDescription className="mt-1">
                        New customer acquisition and churn analysis
                      </CardDescription>
                    </div>
                  </div>
                </CardHeader>
                ...
                {dailyRecent.length > 0 ? (
                  <div className="space-y-4">
                    {dailyRecent.map((data: any, index: number) => {
                      ...
                      return (
                        <div
                          key={index}
                          className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:border-purple-300 hover:shadow-md transition-all duration-200 bg-white"
                        >
                          <div className="flex items-center space-x-3">
                            <div className="p-2 bg-gradient-to-br from-blue-500 to-indigo-500 rounded-lg">
                  ...
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-sm mt-1">
                      Data will appear once customers are added within the
                      selected period
                    </p>
                  </div>
                )}
          ...
          <TabsContent value="customers" className="space-y-6 mt-0 p-6">
            {/* Customer Analytics */}
              ...
              <Card className="border-0 shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300">
                ...
                <CardContent className="p-6 pt-0">
                  <div className="text-2xl font-bold">
                    {customerAnalytics?.acquisition?.current ??
                      acquisitionCount}
                  </div>
                  <div className="flex items-center space-x-1 text-xs mt-1">
                    {customerAnalytics?.acquisition?.growth !== undefined &&
                    customerAnalytics.acquisition.growth > 0 ? (
                      <>
                        <ArrowUpRight className="h-3 w-3 text-green-600" />
                        <span className="text-green-600">
                          +{Math.round(customerAnalytics.acquisition.growth)}%
                        </span>
                      </>
                    ) : customerAnalytics?.acquisition?.growth !== undefined &&
                      customerAnalytics.acquisition.growth < 0 ? (
              ...
              <Card className="border-0 shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300">
                ...
                <CardContent className="p-6 pt-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    MRR churn:{" "}
                    {customerAnalytics?.churn?.mrrChurnRate !== undefined
            ...
            <Card className="border-0 shadow-xl overflow-hidden">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <CardDescription className="mt-1">
                          Daily customer acquisition and retention trends
                        </CardDescription>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <Label
                        htmlFor="growth-range"
                      ...
                      <Select
                        value={growthRange}
                        onValueChange={(v) => setGrowthRange(v)}
                      >
                        <SelectTrigger
                          id="growth-range"
                          className="h-9 w-[140px] border-gray-200 focus:border-purple-300 focus:ring-purple-200"
                        >
              ...
              <CardContent className="p-6">
                {customerAnalytics?.dailyGrowth &&
                customerAnalytics.dailyGrowth.length > 0 ? (
                  <div className="space-y-4">
                    {(() => {
                      const total = customerAnalytics.dailyGrowth?.length || 0;
                      const totalPages = Math.max(
                        1,
                        Math.ceil(total / growthPageSize)
                      );
                      ...
                      return customerAnalytics.dailyGrowth
                        .slice(startIdx, endIdx)
                        .map((data: any, index: number) => (
                          <div
                            key={index}
                            className="grid grid-cols-5 gap-4 p-4 border border-gray-200 rounded-lg hover:border-purple-300 hover:shadow-md transition-all duration-200 bg-white"
                          >
                            <div className="text-center">
                              <p className="font-medium">
                                {new Date(data.date).toLocaleDateString()}
                      ...
                      <span className="text-sm text-gray-600 font-medium">
                        ...
                        {Math.min(
                          ...
                          Math.max(
                            1,
                            Math.ceil(
                              (dailyStats?.length || 0) / growthPageSize
                            )
                          )
                        )}{" "}
                        of{" "}
                        {Math.max(
                          1,
                          Math.ceil((dailyStats?.length || 0) / growthPageSize)
                        )}
                      </span>
                      <Button
                        variant="outline"
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Highest value customers and their performance
                </CardDescription>
              </CardHeader>
              <CardContent>
                ...
                topCustomersByRevenue.length > 0 ? (
                  <Table>
                    ...
                    <TableBody>
                      {(
                        customerAnalytics?.topCustomers || topCustomersByRevenue
                        ...
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            {formatCurrency(
                              customer.mrr || customer.revenue || 0
                            )}
                          </TableCell>
                  ...
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-sm mt-1">
                      Top customers will appear once revenue data is available
                    </p>
                  </div>
                )}
              </CardContent>
          ...
          <TabsContent value="usage" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Usage statistics for key platform features
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
          ...
          <TabsContent value="performance" className="space-y-6">
            ...
            <div className="grid md:grid-cols-4 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {systemHealth?.errorLogs !== undefined
                      ? `${systemHealth.errorLogs}`
                      : "—"}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    System error rate
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Customer support metrics and ticket analytics
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Service quality and reliability indicators
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between p-3 border rounded-lg">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Pre-configured analytical reports
                  </CardDescription>
                </CardHeader>
                  ...
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          System health and support metrics
                        </p>
                      </div>
                    </div>
                    <Button variant="outline" size="sm">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      id="report-name"
                      placeholder="e.g., Q3 Performance Analysis"
                      value={reportName}
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={reportMetric}
                      onValueChange={(v) => setReportMetric(v)}
                    >
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="plan_distribution">
                          Plan Distribution
                        </SelectItem>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={reportRange}
                      onValueChange={(v) => setReportRange(v)}
                    >
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-green-800 mt-1">
                          Professional plan customers show 23% higher
                          engagement. Consider targeted upselling campaigns to
                          Basic plan users.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-blue-800 mt-1">
                          Customers using 3+ features have 40% lower churn rate.
                          Implement feature adoption campaigns for new users.
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-yellow-800 mt-1">
                          68% of users access via mobile. Prioritize mobile
                          feature development to improve user experience.
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-purple-800 mt-1">
                          Response time improved 15% this month. Consider
                          expanding support team to maintain quality during
                          growth.
                        </p>
                      </div>
                    </div>
