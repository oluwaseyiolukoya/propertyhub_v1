...
export async function calculateProjectCashFlow(
  ...
  try {
    ...
    if (funding.length > 0) {
      funding.forEach(f => {
        console.log(`  - Funding: amount=${f.amount}, receivedDate=${f.receivedDate}, status=${f.status}`);
      });
    }
    ...
    funding.forEach(fund => {
      ...
      console.log(`💰 Processing funding: amount=${fund.amount}, receivedDate=${fund.receivedDate}, periodKey=${periodKey}, periodExists=${!!data}`);
    ...
    // Debug: Log totals
    const totalInflow = result.reduce((sum, item) => sum + item.inflow, 0);
    const totalOutflow = result.reduce((sum, item) => sum + item.outflow, 0);
    console.log(`✅ Calculated cash flow for ${result.length} periods`);
    console.log(`📊 Totals: Inflow=${totalInflow}, Outflow=${totalOutflow}, Net=${totalInflow - totalOutflow}`);

    // Debug: Log periods with inflow
    const periodsWithInflow = result.filter(r => r.inflow > 0);
    if (periodsWithInflow.length > 0) {
      console.log(`💰 Periods with inflow:`, periodsWithInflow.map(p => `${p.month}: ${p.inflow}`));
    } else {
      console.log(`⚠️ No periods have inflow - this might indicate a problem`);
    }
