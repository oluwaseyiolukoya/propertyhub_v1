 ...
 * Request a temporary password reset
 */
  ...
  try {
    ...
    if (!account) {
      ...
      return res.json({
        ...
        emailNotFound: true // Internal flag (not shown to user in production)
      });
    }
    ...
    try {
      switch (tableName) {
        ...
        case 'customers':
          // Note: Customers table doesn't have password field in schema
          // This might need to be handled differently or skipped
        ...
        case 'onboarding_applications':
          // Note: Applications table doesn't have password field
