  ...
  try {
    ...
    const expense = await prisma.expenses.create({
      data: {
        ...
        visibleToManager: visibleToManager !== undefined ? visibleToManager : false, // Default false (hidden from managers)
        createdAt: new Date(),
        updatedAt: new Date()
      },
      include: {
...
router.delete('/:id', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!isOwner || expense.property.ownerId !== userId) {
      return res.status(403).json({ error: 'Only property owners can delete expenses' });
    }
...
router.post('/:id/approve', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!isOwner) {
      return res.status(403).json({ error: 'Only owners can approve expenses' });
    }
    ...
    if (!expense.requiresApproval) {
      return res.status(400).json({ error: 'This expense does not require approval' });
    }
