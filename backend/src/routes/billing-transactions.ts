...
router.get('/', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const invoiceTransactions = invoices.map(inv => ({
      ...
      status: inv.status === 'paid' ? 'completed' :
              inv.status === 'refunded' || inv.status === 'partially_refunded' ? 'refunded' :
              'pending',
      date: inv.createdAt,
    ...
    const paymentTransactions = payments.map(pay => ({
      ...
      status: pay.status === 'completed' ? 'completed' :
              pay.status === 'failed' ? 'failed' :
              'pending',
      date: pay.createdAt,
      invoice: pay.providerReference || `PAY-${pay.id.slice(0, 8).toUpperCase()}`,
      description: `${pay.type} payment${pay.properties ? ` for ${pay.properties.name}` : ''}${pay.units ? ` - Unit ${pay.units.unitNumber}` : ''}`,
    ...
    const totalAmount = limitedTransactions.reduce((sum, tx) => sum + tx.amount, 0);
    const completedCount = limitedTransactions.filter(tx => tx.status === 'completed').length;
    const pendingCount = limitedTransactions.filter(tx => tx.status === 'pending').length;
    const failedCount = limitedTransactions.filter(tx => tx.status === 'failed').length;
