    ...
    try {
      ...
      const propertiesWithStats = await Promise.all(
        properties.map(async (property) => {
          ...
          // Get occupied units with their features to check billing cycle
          ...
          // Calculate total monthly income considering billing cycle (annual vs monthly)
...
router.get("/:id", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const stats = {
      ...
      occupiedUnits: property.units.filter((u) => u.status === "occupied")
        .length,
      vacantUnits: property.units.filter((u) => u.status === "vacant").length,
      maintenanceUnits: property.units.filter((u) => u.status === "maintenance")
        .length,
...
router.post("/", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (role !== "owner") {
      return res
        .status(403)
        .json({ error: "Only property owners can create properties" });
    }
  ...
  try {
    ...
    const toInt = (v: any) =>
      v === "" || v === null || v === undefined
        ? undefined
        : Number.parseInt(v as any);
    const toFloat = (v: any) =>
      v === "" || v === null || v === undefined
        ? undefined
        : Number.parseFloat(v as any);
    ...
    const coercedNumeric: Record<string, any> = {
      ...
      currentValue: toFloat(currentValue),
    ...
    if (managerId !== undefined) {
      console.log(
        `🔄 Manager assignment update requested for property ${id}, new managerId: ${
          managerId || "none"
        }`
      );
      ...
      const currentManagerId =
        currentAssignments.length > 0 ? currentAssignments[0].managerId : null;
      ...
      if (managerId !== currentManagerId) {
        ...
        if (currentAssignments.length > 0) {
          console.log(
            `➖ Unassigning ${currentAssignments.length} existing manager(s)`
          );
...
router.delete("/:id", async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.log(
      `🗑️ Delete property request - User: ${userId}, Role: ${role}, Property: ${id}`
    );
    ...
    // 2. Check if manager has delete permission
      ...
      if (
        managerAssignment &&
        managerAssignment.permissions?.canDelete === true
      ) {
        hasDeletePermission = true;
        console.log("✅ Delete authorized: Manager has delete permission");
      } else {
        console.log(
          "❌ Delete denied: Manager does not have delete permission"
        );
      }
    }
    // 3. Admins can delete
...
router.get("/:id/analytics", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const potentialMonthlyIncome = property.units.reduce(
