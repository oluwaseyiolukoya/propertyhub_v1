...
const uploadFavicon = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (!allowedTypes.includes(file.mimetype)) {
      return cb(
        new Error("Only ICO, PNG, and SVG files are allowed for favicon")
      );
    }
    cb(null, true);
  },
...
router.get("/settings/:key", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!setting && (key === "brand_logo_url" || key === "platform_logo_url")) {
      const aliasKey =
        key === "brand_logo_url" ? "platform_logo_url" : "brand_logo_url";
      setting = await prisma.system_settings.findUnique({
        where: { key: aliasKey },
      });
      if (setting) {
...
router.delete(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (
        normalizedKey === "platform_logo_url" ||
        normalizedKey === "platform_favicon_url"
      ) {
        try {
          ...
          console.error(
            `[DELETE ${normalizedKey}] File cleanup error:`,
            fileErr
          );
        }
      }
      ...
      return res
        .status(500)
        .json({ error: "Failed to delete setting", details: error?.message });
    }
  }
);
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to upload favicon" });
    }
  }
);
    ...
    try {
      ...
      if (setting.value && typeof setting.value === "string") {
        try {
          ...
          console.error(
            "[DELETE LOGO] File deletion error (continuing anyway):",
            fileError
          );
        }
      }
    ...
    try {
      ...
      try {
        ...
        if (pathStr) {
          ...
          console.log(
            "[DELETE FAVICON] Setting value not a string path, skipping file deletion"
          );
        }
      } catch (fileErr) {
        console.error(
          "[DELETE FAVICON] File deletion error (continuing anyway):",
          fileErr
        );
      }
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch payment gateway configuration" });
    }
  }
);
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to save payment gateway configuration" });
    }
  }
);
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch payment gateway status" });
    }
  }
);
