...
router.post('/start', authMiddleware, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  console.log(`[Verification Start] Creating request for ${customerEmail} (${customerType})`);
  ...
  if (existingRequest) {
    console.log(`[Verification Start] Using existing request: ${existingRequest.id}`);
    return res.json({
      success: true,
 ...
 * Uses JWT authentication
 */
router.post('/kyc/submit', authMiddleware, asyncHandler(async (req: AuthRequest, res: Response) => {
  const user = req.user;
  ...
  console.log(`[KYC Submit] Creating verification request for ${customerEmail} (${customerType})`);
 ...
 * Uses JWT authentication
 */
router.post('/kyc/upload/:requestId', authMiddleware, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (verificationRequest.customerId !== user.customerId && verificationRequest.customerId !== user.id) {
    return res.status(403).json({ error: 'Not authorized to upload to this request' });
  }
...
router.post('/upload', authMiddleware, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  multerUpload(req, res, async (err) => {
    ...
    console.log(`[KYC Upload] Uploading ${documentType} for request ${requestId}`);
 ...
 * Uses JWT authentication
 */
router.get('/kyc/status', authMiddleware, asyncHandler(async (req: AuthRequest, res: Response) => {
  const user = req.user;
